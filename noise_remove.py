...
class Noise_remover:
    ...
    def get_table_window(self, tokens: list):
        tables = []
        in_table = False
        for token in tokens:
            if '<' in token and 'table' in token and '/' not in token:
                tables.append[[]]
                tables[-1].append(token)
                in_table = True
            if '<' in token and 'table' in token and '/' in token:
                tables[-1].append(token)
                in_table = False
            if not in_table:
                continue
...
test_seed = "https://www.admissions.caltech.edu/apply/first-year-freshman-applicants/deadlines"
